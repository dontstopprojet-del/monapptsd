...
-- admin_alerts
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'admin_alerts' AND column_name = 'recipient_id') THEN
    ALTER TABLE admin_alerts ADD COLUMN recipient_id uuid REFERENCES app_users(id) ON DELETE CASCADE;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'admin_alerts' AND column_name = 'alert_type') THEN
    ALTER TABLE admin_alerts ADD COLUMN alert_type text;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'admin_alerts' AND column_name = 'created_by') THEN
    ALTER TABLE admin_alerts ADD COLUMN created_by uuid REFERENCES app_users(id);
  END IF;
END $$;
